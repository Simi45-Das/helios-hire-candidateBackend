...
const fs = require("fs");
const Photo = require("../models/profileInfo/upoladPhoto");
...
const savePhotoUrls = async ({ urls, userID }) => {
  const photo = new Photo({ urls, userID }); // Ensure both urls and companyId are passed
  await photo.save();
  return photo;
};
...
const getPhotosByuserId = async (userID) => {
  try {
    const photos = await Photo.findOne({ userID });
